...
namespace nn {
	...
	void DropOut::forward(vec::vector2d& forward_propagate, const double ratio, const bool train_flag)
	{
		if (train_flag) {
			...
			const size_t row = forward_propagate.size();
			const size_t col = forward_propagate[0].size();
	...
	namespace ACTIVATION {
		...
		void ReLU::forward(vec::vector2d& forward_propagate)
		{
			...
			const size_t max_a = forward_propagate.size();
			const size_t max_b = forward_propagate[0].size();
			for (size_t i = 0; i < max_a; ++i) {
				for (size_t j = 0; j < max_b; ++j) {
		...
		void ReLU::backward(vec::vector2d& backward_propagate)
		{
			...
			const size_t max_a = Memory_mask[count].size();
			const size_t max_b = Memory_mask[count][0].size();
			for (size_t i = 0; i < max_a; ++i) {
				for (size_t j = 0; j < max_b; ++j) {
		...
		void Softmax::backward(vec::vector2d& backward_propagate)
		{
			...
			for (size_t i = 0; i < row; ++i) {
				for (size_t j = 0; j < col; ++j) {
					backward_propagate[i][j] 
						= (Memory_soft[count][i][j] - backward_propagate[i][j]) / (double)row;
				}
			}
		}
